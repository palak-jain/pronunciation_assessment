...
if [ -z $loc ]; then
    ...
    if uname -a | grep 64 >/dev/null; then
        sdir=$KALDI_ROOT/tools/srilm/bin/i686-m64 
            ...
            sdir=$KALDI_ROOT/tools/srilm/bin/macosx   # [MODIFY] Check the path here
      fi
...
local=data/local
mkdir $local/tmp
touch $local/tmp/lm.arpa    # [MODIFY] Required to create this file
...
gunzip -c exp/tri1/decode/lat.1.gz > exp/tri1/decode/1.lat
lattice-to-post ark:exp/tri1/decode/1.lat ark:- > exp/tri1/decode/1.post
post-to-feats --post-dim=187 ark:exp/tri1/decode/1.post ark,t:1.feat
